...
public class EClientSocket {
    ...
    protected void connectionError() {
        this.m_anyWrapper.error(EClientErrors.NO_VALID_ID, EClientErrors.CONNECT_FAIL.code(), EClientErrors.CONNECT_FAIL.msg());
        this.m_reader = null;
    }

    protected String checkConnected(String host) {
        if (this.m_connected) {
            this.m_anyWrapper.error(EClientErrors.NO_VALID_ID, EClientErrors.ALREADY_CONNECTED.code(), EClientErrors.ALREADY_CONNECTED.msg());
            return null;
        }
        if (isNull(host)) {
            host = "127.0.0.1";
    ...
    public synchronized void eConnect(Socket socket, int clientId) throws IOException {
        ...
        logger.debug("Server Version:" + this.m_serverVersion);
        if (this.m_serverVersion >= 20) {
            this.m_TwsTime = this.m_reader.readStr();
            logger.debug("TWS Time at connection:" + this.m_TwsTime);
        }
        if (this.m_serverVersion < SERVER_VERSION) {
            eDisconnect();
            this.m_anyWrapper.error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS.code(), EClientErrors.UPDATE_TWS.msg());
            return;
        }
    ...
    public synchronized void reqMktData(int tickerId, Contract contract, String genericTickList, boolean snapshot) {
        ...
        if (this.m_serverVersion < MIN_SERVER_VER_SNAPSHOT_MKT_DATA && snapshot) {
        ...
        try {
            ...
            if (this.m_serverVersion >= MIN_SERVER_VER_SNAPSHOT_MKT_DATA) {
                send(snapshot);
            }
        } catch (Exception e) {
    ...
    public synchronized void reqContractDetails(int reqId, Contract contract) {
        ...
        try {
            ...
            if (this.m_serverVersion >= MIN_SERVER_VER_CONTRACT_DATA_CHAIN) {
                send(reqId);
            }
    ...
    public synchronized void placeOrder(int id, Contract contract, Order order) {
        ...
        if (this.m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS) {
        ...
        if (this.m_serverVersion < MIN_SERVER_VER_SSHORT_COMBO_LEGS) {
        ...
        if (this.m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS2) {
        ...
        try {
           ...
           if (this.m_serverVersion >= 22) {
            ...
            if (this.m_serverVersion >= 26) { // Volatility orders
            ...
            if (this.m_serverVersion >= 30) { // TRAIL_STOP_LIMIT stop price
    ...
    public synchronized void reqFundamentalData(int reqId, Contract contract, String reportType) {
        ...
        if (this.m_serverVersion < MIN_SERVER_VER_FUNDAMENTAL_DATA) {
