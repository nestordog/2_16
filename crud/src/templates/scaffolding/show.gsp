<% // AlgoTrader: line 10 remove Impl from classname %>
<% // AlgoTrader: line 32 - 34 only show if display is true %>
<% // AlgoTrader: line 42 - 46 correct display of Maps %>
<% // AlgoTrader: line 54 format date  %>
			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<li class="fieldcontain">
					...
					<%  } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
						<span class="property-value" aria-labelledby="${p.name}-label"><g:formatDate date="\${${propertyName}?.${p.name}}" <% if (cp.format) { %> format="${cp.format}" <% } %>/></span>
