...
CREATE TABLE "security" (
  ...
  UNIQUE KEY `LMAXID_UNIQUE` (`LMAXID`),
  UNIQUE KEY `TTID_UNIQUE` (`TTID`),
  UNIQUE KEY `FUTURE_BK_CONSTRAINT` (`CLASS`,`SECURITY_FAMILY_FK`,`MONTH_YEAR`),
  UNIQUE KEY `UUID_UNIQUE_CONSTRAINT` (`UUID`),
...
CREATE TABLE "easy_to_borrow" (
  ...
  KEY (`STOCK_FK`),
  CONSTRAINT `EASY_TO_BORROW_STOCK_FKC` FOREIGN KEY (`STOCK_FK`) REFERENCES "security" (`ID`)
) ;
