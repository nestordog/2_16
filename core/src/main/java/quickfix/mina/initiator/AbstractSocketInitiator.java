 ...
 *
 * This file may be distributed under the terms of the quickfixengine.org license as defined by quickfixengine.org and appearing in the file LICENSE included in
...
public abstract class AbstractSocketInitiator extends SessionConnector implements Initiator {
    ...
    private void createInitiator(final Session session) throws ConfigError, FieldConvertError {
        ...
        final NetworkingOptions networkingOptions = new NetworkingOptions(getSettings().getSessionProperties(sessionID, true));

        boolean sslEnabled = false;
        if (getSettings().isSetting(sessionID, SSLSupport.SETTING_USE_SSL)) {
            sslEnabled = BooleanConverter.convert(getSettings().getString(sessionID, SSLSupport.SETTING_USE_SSL));
        }
        ...
        final String[] enableProtocole = strEnableProtocole != null ? strEnableProtocole.split(",") : null;
        ...
        final IoSessionInitiator ioSessionInitiator = new IoSessionInitiator(session, socketAddresses, localAddress, reconnectingIntervals, getScheduledExecutorService(), networkingOptions,
                getEventHandlingStrategy(), getIoFilterChainBuilder(), sslEnabled, keyStoreName, keyStorePassword, enableProtocole, cipherSuites);

        this.initiators.add(ioSessionInitiator);
    }

    //QFJ-482
    private SocketAddress getLocalAddress(SessionSettings settings, final SessionID sessionID) throws ConfigError, FieldConvertError {
        ...
        if (settings.isSetting(sessionID, Initiator.SETTING_SOCKET_LOCAL_HOST)) {
            ...
            if (this.log.isInfoEnabled()) {
                this.log.info("Using initiator local host: " + localAddress);
            }
        }
        return localAddress;
    }
    ...
    private SocketAddress[] getSocketAddresses(SessionID sessionID) throws ConfigError {
        ...
        for (int index = 0;; index++) {
            try {
                final String protocolKey = Initiator.SETTING_SOCKET_CONNECT_PROTOCOL + (index == 0 ? "" : Integer.toString(index));
                final String hostKey = Initiator.SETTING_SOCKET_CONNECT_HOST + (index == 0 ? "" : Integer.toString(index));
                final String portKey = Initiator.SETTING_SOCKET_CONNECT_PORT + (index == 0 ? "" : Integer.toString(index));
                TransportType transportType = TransportType.SOCKET;
                if (settings.isSetting(sessionID, protocolKey)) {
                    try {
                        transportType = TransportType.getInstance(settings.getString(sessionID, protocolKey));
    ...
    private boolean isInitiatorSession(Object sectionKey) throws ConfigError, FieldConvertError {
        ...
        return !settings.isSetting((SessionID) sectionKey, SessionFactory.SETTING_CONNECTION_TYPE)
                || settings.getString((SessionID) sectionKey, SessionFactory.SETTING_CONNECTION_TYPE).equals("initiator");
    }

    protected void startInitiators() {
        startSessionTimer();
        for (final IoSessionInitiator initiator : this.initiators) {
            initiator.start();
        }
    }
