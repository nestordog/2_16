...
public class OrderStatusDaoImpl extends OrderStatusDaoBase {
    ...
    @SuppressWarnings("unchecked")
    @Override
    protected OrderStatusVO handleFindOrderStatusByNumber(int number) throws Exception {

        Pair<Order, Map<String, ?>> pair = (Pair<Order, Map<String, ?>>) EsperManager.executeSingelObjectQuery(StrategyImpl.BASE, "select * from OpenOrderWindow where number = " + number);
    ...
    private OrderStatusVO convertPairToOrderStatusVO(Pair<Order, Map<String, ?>> pair) {
        ...
        orderStatusVO.setBroker(order.getBroker() != null ? order.getBroker().getValue() : "");
        orderStatusVO.setNumber(order.getNumber());
