    ...
    private double getCashBalanceDoubleInternal(Collection<Transaction> transactions, Collection<Position> openPositions, Date date) {
        ...
        for (Position openPosition : openPositions) {
            ...
            if (security instanceof Forex) {
                List<Tick> ticks = getTickDao().findTicksByMaxDate(1, 1, security.getId(), date, this.intervalDays);
                if (ticks.isEmpty()) {
                    ticks = getTickDao().findTicksByMinDate(1, 1, security.getId(), date, this.intervalDays);
                    if (ticks.isEmpty()) {
