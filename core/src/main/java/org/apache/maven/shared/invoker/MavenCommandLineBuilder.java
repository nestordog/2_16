// line 104 - 106: go silent if not in debug mode
    ...
    protected void checkRequiredState() throws IOException {
        ...
        // && ( System.getenv( "M2_HOME" ) != null ) )
        {
            if (!getSystemEnvVars().containsKey("M2_HOME")) {
                throw new IllegalStateException("Maven application directory was not " + "specified, and ${maven.home} is not provided in the system "
    ...
    protected File findMavenExecutable() throws CommandLineConfigurationException, IOException {
        ...
        if (this.mvnCommand == null) {
            if (Os.isFamily("windows")) {
                this.mvnCommand = new File(this.mavenHome, "/bin/mvn.bat");
            } else {
                this.mvnCommand = new File(this.mavenHome, "/bin/mvn");
            }

            try {
                File canonicalMvn = this.mvnCommand.getCanonicalFile();
                this.mvnCommand = canonicalMvn;
            } catch (IOException e) {
